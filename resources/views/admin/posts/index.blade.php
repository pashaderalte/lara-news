...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="mt-8">
            <a href="{{ route("admin.posts.create") }}" class="text-indigo-600 hover:text-indigo-900">Add New</a>
        </div>
            ...
            <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                <div
                    ...
                    <table class="min-w-full">
                        <thead>
                        <tr>
                            <th class="px-6 py-3 border-b border-gray-200 bg-violet-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                Title
                            </th>
                            <th class="px-6 py-3 border-b border-gray-200 bg-fuchsia-50"></th>
                        </tr>
                        </thead>

                        <tbody class="bg-fuchsia-50">
                        ...
                        @foreach($posts as $post)
                            <tr>
                                ...
                                <td class="flex justify-end px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                    ...
                                    <form action="{{ route("admin.posts.edit", $post->id) }}" method="GET" class="pl-3">
                                        <button class="p-2 rounded-lg hover:bg-indigo-100">
                                    ...
                                    <form action="{{ route("admin.posts.destroy", $post->id) }}" method="POST" class="pl-3">
                                        @csrf

                                        @method('DELETE')

                                        <button type="button" onclick="confirm('Sure?') ? this.form.submit() : null" class="p-2 rounded-lg hover:bg-red-100">
                                            <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none"
                                                 xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
